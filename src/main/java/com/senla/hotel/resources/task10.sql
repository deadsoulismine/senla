create table if not exists com.senla.hotel.guest
(
    id         serial not null
        constraint guest_pk
            primary key,
    ...
    idservice  integer
        constraint guest_service_id_fk
            references com.senla.hotel.service
);

alter table hotel.guest
    owner to postgres;

create table if not exists com.senla.hotel.service
(
	id serial not null
...
create table if not exists com.senla.hotel.room
(
	idguest integer not null
		constraint room_fk
			references com.senla.hotel.guest,
	number integer,
	price integer,
